...
const ProductModalSeller: React.FC<ProductModalProps> = ({show, onClose, onSubmit, product}) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement> | { target: { name: string, value: any } }) => {
        ...
        // Arrays for tags/materials
            ...
            setForm(prev => ({ ...prev, [name]: Array.isArray(value) ? value : value.split(',').map((t: string) => t.trim()) }));
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50" onClick={handleBackdropClick}>
            <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex flex-col items-center mb-4">
                    {form.image ? (
                        <img src={typeof form.image === 'string' ? form.image : URL.createObjectURL(form.image)} alt="Product" className="w-40 h-40 object-cover rounded mb-2 border" />
                    ) : (
                        <div className="w-40 h-40 bg-[#E8C07D] rounded flex flex-col items-center justify-center text-[#6B4F3B] mb-2 relative">
                            <span className="mb-2">No Image</span>
                            <label className="absolute bottom-2 right-2 cursor-pointer bg-white rounded-full p-1 shadow hover:bg-[#E8C07D] transition" title="Upload image">
                ...
                <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                    ...
                    <div className="flex gap-2">
                        <input type="number" name="width" placeholder="Width (cm)"
                               ...
                               onChange={e => handleChange({ target: { name: 'dimensions', value: { ...form.dimensions, width: Number(e.target.value) } } })}/>
                        <input type="number" name="height" placeholder="Height (cm)"
                               className="border rounded px-3 py-2 w-1/3" value={form.dimensions?.height || ''}
                               onChange={e => handleChange({ target: { name: 'dimensions', value: { ...form.dimensions, height: Number(e.target.value) } } })}/>
                        <input type="number" name="depth" placeholder="Depth (cm)"
                               className="border rounded px-3 py-2 w-1/3" value={form.dimensions?.depth || ''}
                               onChange={e => handleChange({ target: { name: 'dimensions', value: { ...form.dimensions, depth: Number(e.target.value) } } })}/>
                    </div>
                    <button type="submit"
